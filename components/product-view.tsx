...
export function ProductView({ verificadorData, amazonData }: ProductViewProps) {
  ...
  const getPriceStats = () => {
    if (!verificadorData.has_serie_historica || verificadorData.serie_historica.length === 0) {
      // Si no hay histórico, usar el precio destacado
      const currentPrice = verificadorData.precios_destacados[0]?.precio;
      const numCurrentPrice = currentPrice ? parseFloat(currentPrice.replace(/[€,\s]/g, '').replace(',', '.')) : 0;
    ...
    // El precio actual debe ser el último del histórico
    const sortedData = [...verificadorData.serie_historica].sort((a, b) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime());
    const currentPrice = sortedData[0]?.precio || prices[prices.length - 1];
          ...
          <div className="space-y-4">
            ...
            {amazonData?.product && (
              ...
              >
                ...
                {isAnalyzing ? "Analizando..." : analysis ? "Análisis Completado ✓" : "Análisis con IA"}
              </Button>
            )}
